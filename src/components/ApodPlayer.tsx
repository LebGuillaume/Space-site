    ...
    return (
        <>
            <div className="w-full mx-auto flex justify-between">
                <button className="cursor-pointer  mx-4" onClick={prevHandler} disabled={day === 0}>
                    <CircleChevronLeft size={36}
                                       className={`transition-all  ${day !== 0 ? "hover:scale-110 hover:text" : ""}`}
                                       color="var(--clr-violet-light)"></CircleChevronLeft>
                </button>
                {!isLoading ? (<div className="h-[400px] w-full">
                    {media_type === "video" ? <iframe height="100%" width="100%" src={url}></iframe> :
                        <img src={url} alt="apod-img" className="h-full w-full object-cover"/>}
                </div>) : (
                ...
                <button onClick={prevHandler}>
                    <CircleChevronRight className="cursor-pointer hover:scale-110 hover:text  mx-4" size={36}
                                        color="var(--clr-violet-light)"></CircleChevronRight>
                </button>
            </div>
